...
abstract class ServiceAbstract implements ServiceInterface {
    ...
    public function __construct(?Config $config = null, ?PayloadInterface $payload = null, ?WebhookInterface $webhook = null, ?MessageInterface $message = null) {
        ...
        // set payload if provided, else create through factory if class is defined
        if($payload !== null) {
        ...
        // set message if provided, else create through factory if class is defined
        if($message !== null) {
            $this->setMessage($message);
        } elseif ($message === null && $this->messageFactoryClass !== null && $this->webhook !== null) {
            $this->setMessageFactory()->setMessageThroughFactory($this->payload?->event ?? '', $this->webhook);
        }
    }
    ...
    // PAYLOAD MANAGEMENT
        ...
        final public function setPayloadFactory(?string $payloadFactoryClass = null): self {
            ...
            if($payloadFactoryClass === null) {
                throw new \InvalidArgumentException("No payload factory class provided.");
            }
            
            // class does not exist
            if(!class_exists($payloadFactoryClass)) {
                throw new \InvalidArgumentException("Payload factory class does not exist: {$payloadFactoryClass}");
            }
        ...
        final public function setPayloadThroughFactory(string $payload): self {
            // ensure payload factory is set
            if(!isset($this->payloadFactory)) {
                throw new \InvalidArgumentException("Payload factory is not set.");
            }
    ...
    // MESSAGE MANAGEMENT
        ...
        final public function setMessage(?MessageInterface $message = null): self {
        ...
        final public function setMessageThroughFactory(string $event): self {
            // ensure message factory is set
            if(!isset($this->messageFactory)) {
                throw new \InvalidArgumentException("Message factory is not set.");
            }
